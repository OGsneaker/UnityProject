...
public class playerController : MonoBehaviour
{
    ...
    void Update()
    {
        if (healthCount > 0)
        {
            ...
            if (attacking == false && hurting == false)
            {
                ...
                // Hide all weapons during movement/idle
                if (attacking == false)
                {
                    swordDown.SetActive(false);
                    swordLeft.SetActive(false);
            ...
            // ✅ FIXED ATTACK CODE BELOW
            if (Input.GetKeyDown(KeyCode.Space) && attackCoolDownTime <= 0 && hurting == false)
            {
                attacking = true;
                justAttacked = true;
                ...
                {
                    playerAnimatons.Play("attackBow");
    ...
    // ✅ coroutine to end attack animation
